...
function logo_markup( $data ) {
	if ( $data['type'] == 'svg' ) :
		$logo = "<div id='logo'><a href='{$data['uri']}'>{$data['file']}</a></div>";

	elseif ( in_array( $data['type'], [ 'png', 'media-library', 'jpg' ] ) ):
		$logo = "<div id='logo'><a href='{$data['uri']}'><img src='{$data['src']}' alt='{$data['alt']}'></a></div>";

	else :
		$logo = "<div id='logo'><a href='{$data['uri']}'>{$data['alt']}</a></div>";
